...
router.post('/login', function (req, res) {
  ...
  var options = {
    url: 'http://192.168.14.40:5000/user/login',
    method: 'POST',
    json: true,
    headers: {
  ...
  request(options, function (error, response, data) {
    ...
    if(data.errcode == '0'){
      ...
      res.cookie('token',data.result, { expires: new Date(Date.now() + 900000), httpOnly: true });
...
router.post('/register', function (req, res) {
  ...
  var options = {
    url: 'http://192.168.14.40:5000/user/regist',
    method: 'POST',
    json: true,
    headers: {
      "content-type": "application/json",
...
router.get('/home', function (req, res, next) {
  ...
  request(options, function (error, response, data) {
    ...
    if(data.result == '1'){
      ...
      var page_total =  Math.ceil(data.msg.total/10);
      res.render('home', {
        page: page,
        data: data.msg,
        error: '',
...
router.post('/release', urlencodedParser, function(req, res, next) {
  console.log(req.body);
  var options = {
    url: 'http://192.168.14.40:5000/blog/publish',
    method: 'POST',
    json: true,
    headers: {
      "content-type": "application/json",
...
router.post('/pay', urlencodedParser, function(req, res, next) {
  console.log(req.body);
  var options = {
    url: 'http://192.168.14.40:5000/pay',
    method: 'POST',
    json: true,
    headers: {
      "content-type": "application/json",
